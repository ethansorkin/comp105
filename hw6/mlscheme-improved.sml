...
fun freeIn (LITERAL v) name = false
  | freeIn (VAR n) name = if n = name then true else false
  | freeIn (SET (n, e)) name = if name = n then true else freeIn e name
  | freeIn (IFX (e1, e2, e3)) name = List.exists (fn(e) => freeIn e name) 
                                                                    [e1, e2, e3]
  | freeIn (WHILEX (e, e2)) name = List.exists (fn(x) => freeIn x name) [e, e2]
  | freeIn (BEGIN es) name = List.exists (fn(e) => freeIn e name) es
  | freeIn (APPLY (e, es)) name = List.exists (fn(ex) => freeIn ex name) (e::es)
  | freeIn (LETX (l, [], e1)) name = freeIn e1 name
  | freeIn (LETX (LET, bs, e)) name = List.exists (fn(ex) => freeIn (snd ex) name) bs orelse
                  (freeIn e name andalso not (member (name, List.map fst bs)))
  | freeIn (LETX (LETSTAR, b::bs, e)) name = freeIn (LETX (LET, [b], LETX (LETSTAR, bs, e))) name
  | freeIn (LETX (LETREC, bs, e)) name = not (member (name, List.map fst bs)) andalso 
           (List.exists (fn(ex) => freeIn (snd ex) name) bs orelse freeIn e name)
  | freeIn (LAMBDA (xs, e)) name = (not (member (name, xs))) andalso freeIn e name
    

        val () =
            Unit.checkAssert "part a) free test "
            (fn () => freeIn (LAMBDA (["x"], LAMBDA (["y"], (APPLY ((VAR "equal?"), [(VAR "x"), (VAR "y")]))))) "equal?")

        val () =
            Unit.checkAssert "part a) not free test"
            (fn () => not (freeIn (LAMBDA (["x"], LAMBDA (["y"], (APPLY ((VAR "equal?"), [(VAR "x"), (VAR "y")]))))) "x"))

        val () =
            Unit.checkAssert "part b) free test"
            (fn () => freeIn (LAMBDA (["y"], APPLY ((VAR "equal?"), [(VAR "x"), (VAR "y")]))) "equal?")

        val () =
            Unit.checkAssert "part b) not free test"
            (fn () => not (freeIn (LAMBDA (["y"], APPLY ((VAR "equal?"), [(VAR "x"), (VAR "y")]))) "y"))


        (* additional tests *)
        val () =
          Unit.checkAssert "test"
          (fn () => not (freeIn (LITERAL (BOOLV true)) "t"))

        val () =
          Unit.checkAssert "test2"
          (fn () => freeIn (VAR "x") "x")

        val () =
          Unit.checkAssert "test3"
          (fn () => freeIn (SET ("x", (LITERAL (NUM 3)))) "x")
...
fun improve ((ns, e), env) = ((ns, e), (List.filter (fn(x) => freeIn e (fst x)) env))
